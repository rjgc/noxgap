...
function WPOS() {
    ...
    function checkAppCompatibility(){
        ...
        if (!('localStorage' in window && window.localStorage !== null)) {
            swal("Este browser não suporta localStorage.");
            return false;
        }
        // Check application cache: not required to run
        if (window.applicationCache == null){
            swal("Este browser não suporta applicationCache, não será possivel usar o modo offline");
        }
        return true;
    }
    ...
    this.checkCacheUpdate = function(){
            if (window.applicationCache == null){
                console.log("A Cache de Aplicação não é suportada.");
                WPOS.initLogin();
                return;
            }
            ...
            if (appCache.status == appCache.UNCACHED){
                console.log("A Cache de Aplicação ainda não foi carregada");
                WPOS.initLogin();
                return;
            }
            ...
            appCache.addEventListener('cached', function(e) {
                console.log("A Cache foi carregada, não é nessário Atualizar a pagina");
                WPOS.initLogin();
            });
            // wait for update to finish: check after applying event listener aswell, we may have missed the event.
            appCache.addEventListener('updateready', function(e) {
                console.log("Atualização da Appcache realizada, carregando...");
                setLoadingBar(100, "Loading...");
                appCache.swapCache();
                location.reload(true);
            });
            appCache.addEventListener('noupdate', function(e) {
                console.log("Sem Atualizações da Appcache");
                WPOS.initLogin();
            });
            appCache.addEventListener('progress', function(e) {
                ...
                setLoadingBar(cacheloaded, "Atualizado Aplicação...");
            });
            appCache.addEventListener('downloading', function(e) {
                console.log("Updating Appcache");
                setLoadingBar(1, "Atualizado Aplicação...");
            });
            if (appCache.status == appCache.UPDATEREADY){
                console.log("Atualização da Appcache Concluida, carregando...");
                setLoadingBar(100, "Loading...");
                appCache.swapCache();
                location.reload(true);
            }
    ...
    this.initLogin = function(){
        ...
        if (getDeviceUUID() == null) {
            ...
            swal("O dispositivo ainda não foi configurado, faça login como Administrador e configure o dispositivo.");
            initialsetup = true;
            online = true;
            return false;
        }
    ...
    function deployDefaultScanApp(){
        $.getScript('/assets/js/jquery.scannerdetection.js').done(function(){
            ...
            swal("Houve uma falha a carregar o Plugin de Scanning.");
        });
    }
    ...
    this.lockSession = function(){
        ...
        showLogin("A Sessão está bloqueada, faça login para continuar.", true);
        session_locked = true;
    };
            ...
            if (result === true) {
                ...
                if (initialsetup) {
                    if (isUserAdmin()) {
                        ...
                        swal("Faça Login como administrador para realizar a primeira configuração");
                        showLogin();
                    }
                } else {
                    if (session_locked){
    ...
    this.logout = function () {
        swal({title: "Tem a certeza que pretende fazer Logout?",
            text: "",
            //type: "warning",
            showCancelButton: true,
            confirmButtonColor: "#429ae6",
            confirmButtonText: "Ok"
        }, function() {
            var sales = WPOS.sales.getOfflineSalesNum();
            if (sales>0) {
                swal({title: "Existem vendas offline.\nPretende realizar um backup?",
                    text: "",
                    //type: "warning",
                    showCancelButton: true,
                    confirmButtonColor: "#429ae6",
    ...
    function offlineAuth(username, hashpass) {
        if (localStorage.getItem("wpos_auth") !== null) {
            ...
            if (jsonauth[username] === null || jsonauth[username] === undefined) {
                swal("As suas credenciais não existem de momento em modo offline.");
                return false;
            } else {
            ...
            swal("Não existem credências de momento guardadas localmente.");
            return false;
        }
    }
    ...
    this.deviceSetup = function () {
        ...
        var syear = $("#serieyear").val();
        var sstart = $("#seriestart").val();
        var send = $("#serieend").val();

        // check input
        if ((devid == null && devname == null) || (locid == null && locname == null)) {
            swal("Selecione um item ou especifique um novo.");
        }else if((devname != '' && devname != null && locname != '' && locname != null)){
            if((syear == '' || syear == null) || (sstart == '' || sstart == null) || (send == '' || send == null)) {
                swal("Insira uma Serie de Faturação valida.");
            }else{
                // call the setup function
                deviceSetup(devid, devname, locid, locname, syear, sstart, send, function(result){
                    if (result) {
                        currentuser = null;
                        initialsetup = false;
                        ...
                        swal("Ocorreu um problema ao configurar o dispositivo, por favor tente novamente.");
                    }
                });
            }
        } else {
    ...
    this.seriesvalidation = function () {
        if(($("#posdevices option:selected").val() != '' && $("#posdevices option:selected").val() != null)) {
            $("#serieyear").prop('disabled', true);
            $("#seriestart").prop('disabled', true);
            $("#serieend").prop('disabled', true);
        }else{
            $("#serieyear").prop('disabled', false);
            $("#seriestart").prop('disabled', false);
            $("#serieend").prop('disabled', false);
        }
    };

    function initSetup() {
        $("#loadingbartxt").text("Iniciando a configuração");
        WPOS.util.showLoader();
    ...
    function loadOnlineData(step, loginloader){
        var statusmsg = "The POS está a atualizar os dados e alternando para o modo online.";
        switch (step){
            case 1:
                $("#loadingbartxt").text("Carregando os recursos online");
                // get device info and settings
                setLoadingBar(10, "A obter as configurações do dispositivo...");
                setStatusBar(4, "Atualizando as configurações do dispositivo...", statusmsg, 0);
            ...
            case 2:
                // get stored items
                setLoadingBar(30, "A obter os items armazenados...");
                setStatusBar(4, "A atualizar os items armazenados...", statusmsg, 0);
            ...
            case 3:
                // get customers
                setLoadingBar(60, "A obter os clientes...");
                setStatusBar(4, "A atualizar os clientes...", statusmsg, 0);
            ...
            case 4:
                ...
                setLoadingBar(80, "A obter as vendas recentes...");
                setStatusBar(4, "A atualizar as vendas...", statusmsg, 0);
                fetchSalesTable(function(data){
                    ...
                    startSocket();
                    setStatusBar(1, "NoxPOS Online", "O POS está em mode online.\nAs atualizaçôes em tempo de real estão ativas.", 0);
                    initDataSuccess(loginloader);
                    var offline_num = WPOS.sales.getOfflineSalesNum();
                    if (offline_num>0){
                        $("#backup_btn").show();
                        // check for offline sales on login
                        setTimeout('if (WPOS.sales.uploadOfflineRecords()){ WPOS.setStatusBar(1, "NoxPOS Online"); }', 2000);
                    } else {
                        $("#backup_btn").hide();
                    }
                });
    ...
    function initOfflineData(loginloader){
        ...
        setLoadingBar(50, "A carregar dados offline...");
        loadConfigTable();
        loadItemsTable();
        loadCustTable();
        loadSalesTable();
        swal("A ligação com a Internet não está ativa e o POS foi iniciado em modo offline. \nAlguns recursos não estão disponíveis no modo off-line, mas pode fazer vendas e alterar transações que estão disponíveis localmente. \nQuando a conexão estiver disponível o POS processará as transações para o servidor");
    ...
    function initDataSuccess(loginloader){
        if (loginloader){
            setLoadingBar(100, "Processando os dados...");
            $("title").text("NoxPOS");
            WPOS.initPlugins();
            populateDeviceInfo();
    ...
    this.removeDeviceRegistration = function(){
        ...
        swal("Please login as an administrator to use this feature");
    };

    this.resetLocalConfig = function(){
        if (isUserAdmin()){
            swal({title: "Tem certeza de que deseja restaurar as configurações locais?\n",
                text: "",
                //type: "warning",
                showCancelButton: true,
                confirmButtonColor: "#429ae6",
        ...
        swal("Faça login como administrador para usar esta funcionalidade");
    };
    ...
    this.clearLocalData = function(){
        if (isUserAdmin()){
            ...
            swal({title: "Tem certeza de que deseja apagar todos os dados locais?\nTodos os dados armazenados localmente serão removidos, exceto a chave de registro do dispositivo.\nAs vendas offline também serão excluídas.",
                text: "",
                //type: "warning",
                showCancelButton: true,
                confirmButtonColor: "#429ae6",
    ...
    this.refreshRemoteData = function(){
        swal({title: "Tem certeza que deseja recarregar os dados do servidor?",
            text: "",
            //type: "warning",
            showCancelButton: true,
            confirmButtonColor: "#429ae6",
        ...
        /*var answer = confirm("Tem certeza que deseja recarregar os dados do servidor?");
        if (answer){
            loadOnlineData(1, false);
        }*/
    };
    ...
    function switchToOffline() {
        if (canDoOffline()==true) {
            ...
            online = false;
            setStatusBar(3, "WPOS Offline", "O POS está em modo offline, armazenará os dados localmente até que a conexão fique disponível.", 0);
            ...
            swal('Ocorreu um erro a conectar com o servidor e os dados necessários para executar o modo offline não estão presentes.\nVerifique e sua ligação e tente novamente.');
            showLogin();
            setLoadingBar(100, "Erro a trocar para o modo offline");
            return false;
        }
    }
    ...
    this.sendJsonData = function (action, data) {
        ...
        if (response.status == "200") {
            ...
            if (json == null) {
                swal("Error: The response that was returned from the server could not be parsed!");
                return false;
            }
            ...
            switchToOffline();
            swal("Erro a ligar ao Servidor: \n"+response.statusText+", \nmodo offline ativo.");
            return false;
        }
        } catch (ex) {
            switchToOffline();
            swal("Erro a enviar dados, modo offline ativo.\nException: "+ex.message);
            return false;
        }
    };
            ...
            if (data.id==configtable.deviceid) {
                if (data.hasOwnProperty('a') && (data.a == "removed" || data.a == "disabled")) {
                    ...
                    if (data.a == "removed")
                        removeDeviceUUID();
                    logout();
                    swal("This device has been " + data.a + " by the administrator,\ncontact your device administrator for help.");
                    return;
                }
     ...
     * @returns {boolean}
     */
    function deviceSetup(devid, newdevname, locid, newlocname, syear, sstart, send, callback) {
        var data = {};
        data.uuid = setDeviceUUID(false);
        if (devid === "") {
            data.devicename = newdevname;
            data.serieyear = syear;
            data.seriestart = sstart;
            data.serieend = send;
        } else {
            data.deviceid = devid;
        }
    ...
    function socketError(){
        if (WPOS.isOnline())
            setStatusBar(5, "Update Feed Offline", "O POS está em mode online..\nAs atualizações em tempo real não estão ativas.", 0);
        socketon = false;
        authretry = false;
    }
...
$(function () {
    ...
    if (document.location.host=="noxpos.com" || document.location.host=="alpha.noxpos.com"){
        var login = $("#logindiv");
        login.append('<button class="btn btn-primary btn-sm" onclick="$(\'#username\').val(\'staff\');$(\'#password\').val(\'password\'); WPOS.userLogin();">Demo Login</button>');
        if (document.location.host=="alpha.noxpos.com")
