...
function WPOSItems() {
    ...
    function addItemRow(qty, name, unit, taxid, sitemid, data) {
        ...
        var row = $('<tr class="item_row">' +
            '<td><input class="itemid" type="hidden" value="' + sitemid + '" data-options=\''+JSON.stringify(data)+'\' /><input onChange="WPOS.sales.updateSalesTotal();" style="width: 100%;max-width: 90px;" type="text" class="itemqty numpad" value="' + qty + '" /></td>' +
            ...
            '<td><input '+((disableprice==true && unit!="")?"disabled":"")+' onChange="WPOS.sales.updateSalesTotal();" style="width: 100%;max-width: 90px;" type="text" class="itemunit numpad" value="' + unit + '" /></td>' +
            ...
            '<td><input style="width: 100%;max-width: 90px;" type="text" class="itemprice" value="0.00" disabled /></td>' +
...
function WPOSSales() {
    ...
    function addPaymentRow(method, value, tender, change, extraData){
        ...
        var payrow =  '<tr '+data+'><td>' +
            ...
            exmethod+ '</select>' +
            '<div style="display: inline-block;">Tendered:</div><input onChange="WPOS.sales.updatePaymentChange($(this).parent());" class="paytender numpad" style="width:50px;" type="text" value="'+(method!='cash'?0.00:(tender!=null?tender:value))+'" />' +
            '<div style="display: inline-block;">Change:</div><input class="paychange" style="width:50px;" type="text" value="'+(method!='cash'?0.00:(change!=null?change:0.00))+'" readonly />' +
            '</div></td>' +
    ...
    function processOrder(){
        ...
        if (WPOS.isOnline()) {
            ...
            // update status
            WPOS.setStatusBar(3, "NoxPOS Offline ("+WPOS.sales.getOfflineSalesNum()+" offline records)");
        }
    ...
    function ProcessSaleTransaction(){
        ...
        var cursale = null;
        if (curref!==null){
            //swal("Processing using orders existing reference: "+curref);
    ...
    this.postSaleUpload = function(jsonresponse, callbackref){
        if (jsonresponse !== false) {
            ...
            // reset status Icon
            WPOS.setStatusBar(1, "NoxPOS Online", "The POS is running in online mode.\nThe feed server is connected and receiving realtime updates.", 0);
        } else {
            // ERROR
            if (WPOS.switchToOffline()) { // do not store record if offline mode is not supported.
                ...
                WPOS.setStatusBar(3, "NoxPOS Offline ("+WPOS.sales.getOfflineSalesNum()+" offline records)", statusmsg, 0);
            } else {
                // remove from offline temp
                removeOfflineSale(callbackref);
            }
    ...
    function getSaleObject(){
        ...
        var serieval = null;
        if (localStorage.getItem("wpos_serieval") === null) {
            localStorage.setItem("wpos_serieval", config.deviceconfig.seriestart);
            serieval = config.deviceconfig.seriestart;
        }else if(parseInt(localStorage.getItem("wpos_serieval"))+1 >= parseInt(config.deviceconfig.seriestart) && parseInt(localStorage.getItem("wpos_serieval"))+1 <= parseInt(config.deviceconfig.serieend)){
            serieval = parseInt(localStorage.getItem("wpos_serieval")) + 1;
            localStorage.setItem("wpos_serieval", serieval);
        }else{
            swal('Erro de Serie, contacte o Administrador');
            return;
        }
        salesobj.ref = date + "-" + config.deviceid + "-" + config.deviceconfig.serieyear + "" + serieval;
        /* */
    ...
    this.postVoidUpload = function(jsonresponse, refundobject){
        if (jsonresponse !== false) {
            ...
            addSalesRecord(jsonresponse);
            // reset status Icon
            WPOS.setStatusBar(1, "NoxPOS Online");
        } else {
            // ERROR
            if (WPOS.switchToOffline()) { // do not store record if offline mode is not supported.
        ...
        if (localStorage.getItem("wpos_osales") !== null) {
            ...
            if (count > 0){
                ...
                if (WPOS.switchToOffline()) {
                    // update status
                    WPOS.setStatusBar(3, "NoxPOS Offline ("+count+" offline records)");
                    return false;
                }
            } else {
                $("#backup_btn").hide();
