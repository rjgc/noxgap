...
function WPOSTransactions() {
    ...
    this.recallLastTransaction = function(){
        ...
        if (lastref == null){
            swal("No transactions yet for this session.");
            return;
        }
    ...
    datatable = $('#transactiontable').dataTable(
        {
            ...
            "aoColumns": [
                ...
                { "sType": "html", mData:function(data,type,val){ return "<button class='btn btn-sm btn-primary' onclick='WPOS.trans.showTransactionInfo("+"\""+data.ref+"\""+")'>Ver</button>"; }, "bSortable": false }
            ],
            "columns": [
                {},
                {},
    ...
    function getOfflineStatusHtml(ref){
        ...
        switch (syncstat){
            case 1:
                ...
                break;
            case 2:
                ostathtml = '<span class="label label-sm label-warning arrowed">parcial</span>';
                break;
            case 3:
                ostathtml = '<span class="label label-sm label-primary arrowed">sincronizada</span>';
                break;
        }
        return ostathtml;
    }
    ...
    function getStatusHtml(status){
        ...
        switch(status){
            case 0:
                stathtml='<span class="label label-primary arrowed">Encomenda</span>';
                break;
            case 1:
                stathtml='<span class="label label-success arrowed">Finalizada</span>';
                break;
            case 2:
                stathtml='<span class="label label-danger arrowed">Anulada</span>';
                break;
            case 3:
                stathtml='<span class="label label-warning arrowed">Reembolso</span>';
                break;
            default:
                stathtml='<span class="label arrowed">Desconhecida</span>';
                break
        }
        return stathtml;
    }
    ...
    function populateTransactionInfo(ref){
        ...
        if (record===false){
            swal("Could not find the transaction record!");
        }
    ...
    function populateRefundTable(record){
        ...
        if (record.refunddata !== undefined){
            ...
            for (var i = 0; i<record.refunddata.length; i++){
                ...
                $(refundtable).append('<tr><td><span class="label label-warning arrowed">Refund</span></td><td>'+WPOS.util.getDateFromTimestamp(tempdata.processdt)+'</td><td><button class="btn btn-sm btn-primary" onclick="WPOS.trans.showRefundDialog('+i+');">Ver</button></td></tr>');
            }
        }
        if (record.voiddata !== undefined && record.voiddata !== null){
            $(refundtable).append('<tr><td><span class="label label-danger arrowed">Void</span></td><td>'+WPOS.util.getDateFromTimestamp(record.voiddata.processdt)+'</td><td><button class="btn btn-sm btn-primary" onclick="WPOS.trans.showVoidDialog();">Ver</button></td></tr>');
        }
    }
    ...
    this.searchRemote = function(){
        ...
        if (Object.keys(searchdata).length>0){
            ...
            swal("Please select at least one search option.");
        }
    };
    ...
    this.updateSaleNotes = function(){
        if (WPOS.isOnline()){
            ...
            swal("Updating notes offline is not supported at this time\nsorry for the inconvenience");
        }
    };
